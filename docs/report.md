...
## 1. TIÊU CHÍ ĐÁNH GIÁ & PHÂN TÍCH YÊU CẦU CỦA GIẢNG VIÊN

Dựa trên yêu cầu đánh giá Bài tập lớn môn Big Data (Thang điểm 10 bao gồm 9.0 điểm chuẩn + 1.0 điểm thưởng), đồ án đã thiết lập và hoàn thành đầy đủ một luồng dữ liệu (data pipeline) hoàn chỉnh đáp ứng trọn vẹn các tiêu chí sau:

![Barem điểm và Yêu cầu của Giảng viên](grading_rubric.png)

* **Thu thập dữ liệu (1.0 điểm):** Phải tự lấy dữ liệu thông qua API, cào dữ liệu (crawl), sinh giả lập hoặc dùng kho mở.
  * *Triển khai:* Tích hợp Spotify Web API kết hợp xử lý dữ liệu từ kho mở hơn 28,000 bài hát thông qua `src/data_collector.py`.
* **Lưu trữ & Tiền xử lý (1.5 điểm):** Bắt buộc làm sạch dữ liệu và lưu vào hệ thống phân tán như HDFS hoặc NoSQL (ví dụ: Cassandra), sau đó đọc lại từ các hệ thống này.
  * *Triển khai:* Lưu trữ và làm sạch dữ liệu trong cơ sở dữ liệu NoSQL **MongoDB** (sử dụng hai collection: `tracks` và `processed_tracks`). Dữ liệu được đọc lại trực tiếp từ MongoDB phục vụ cho quá trình huấn luyện Spark và Engine gợi ý.
* **Xử lý bằng Apache Spark (1.5 - 2.5 điểm):** Dùng Spark để phân tích và giải thích rõ ưu/nhược điểm của các bước xử lý.
  * *Triển khai:* Sử dụng **Apache Spark (PySpark MLlib)** thực thi song song thuật toán K-Means và Bisecting K-Means trên tập dữ liệu Parquet, phân tích chi tiết WSSSE Cost qua Elbow Method và so sánh Silhouette Score trong `src/clustering.py`. Các ưu/nhược điểm của Spark đã được ghi nhận chi tiết trong báo cáo.
* **Trực quan hóa/Kết quả (1.5 - 2.5 điểm):**
  * *Triển khai theo Hướng Machine Learning (ML) & Data Engineering:* Thiết lập Dashboard trực quan hóa tương tác thời gian thực bằng **Streamlit** hiển thị:
    * Cấu trúc so sánh đa chiều Radar Chart của bài hát nguồn và bài hát gợi ý.
    * Bản đồ không gian phân cụm PCA dưới cả 2 dạng: **2D (SVG mượt mà)** và **3D Tương tác (WebGL)** kết nối trực quan các điểm nhạc.
    * Hỗ trợ nghe thử trực tiếp bài hát bằng Player nhúng (`iframe`) và trình phát nhạc preview của Streamlit.
* **Điểm thưởng (+1.0 điểm):**
  * *Sự sáng tạo & Công cụ ngoài môn học (+0.5đ):* Thiết lập cơ chế **Offline Fallback** chống chịu lỗi cực mạnh khi Spotify API bị Rate Limit (429) hoặc IP bị chặn, tự động chuyển vùng dữ liệu cục bộ trong 1 giây mà không làm đơ ứng dụng. Đồng thời phát triển thêm chế độ dựng hình **3D Tĩnh (CPU - thông qua Matplotlib)** để dự phòng lỗi WebGL của trình duyệt.
  * *Đóng gói thành sản phẩm demo trực quan (+0.5đ):* Đóng gói toàn bộ mã nguồn chạy dưới dạng ứng dụng Web Streamlit dễ dàng deploy và thao tác chỉ với 1 click chuột.
...
### 3.1. Bối cảnh và Bài toán Gợi ý Âm nhạc
...
### 3.2. Phân tích So sánh: Content-based Filtering vs Collaborative Filtering
...
## 4. KIẾN TRÚC HỆ THỐNG & DATA PIPELINE
...
## 5. THU THẬP DỮ LIỆU & Ý NGHĨA CÁC ĐẶC TRƯNG ÂM HỌC

### 5.1. Quy trình Tích hợp Spotify Web API
...
### 5.2. Định nghĩa và Ý nghĩa Toán học của các Đặc trưng Âm thanh
...
## 6. LƯU TRỮ & TIỀN XỬ LÝ DỮ LIỆU

### 6.1. Lý lý Lựa chọn Cơ sở Dữ liệu MongoDB
...
### 6.2. Quy trình Làm sạch và Chuẩn hóa Dữ liệu (Pandas Pipeline)
...
### 6.3. Kỹ nghệ Đặc trưng (Feature Engineering)
...
## 7. PHÂN CỤM DỮ LIỆU VỚI APACHE SPARK K-MEANS

### 7.1. Lý thuyết thuật toán K-Means phân tán trong Spark ML
...
### 7.2. Xác định Số lượng Cụm tối ưu bằng Phương pháp Khuỷu tay (Elbow Method)
...
### 7.3. So sánh mô hình học máy: Standard K-Means vs Bisecting K-Means
...
### 7.4. Ưu và nhược điểm của các bước xử lý trên Apache Spark
...
### 7.5. Phân tích Đặc điểm Từng Cụm Âm nhạc (Cluster Analysis)
...
### 8.1. Lý thuyết Độ tương đồng Cosine (Cosine Similarity)
...
### 8.2. Thuật toán Gợi ý Tối ưu hóa theo Cụm (Cluster-based Filtering)
...
### 8.3. Cơ chế Phân giải Liên kết Spotify Nâng cao (URL Resolution)
...
### 8.4. Tải Ảnh bìa Album Song song (Parallel Cover Fetcher)
...
## 9. TRỰC QUAN HÓA & GIAO DIỆN DASHBOARD

### 9.1. Bảng điều khiển (Sidebar Settings)
...
### 9.2. Phân tích bài hát nguồn (Plotly Radar Chart)
...
### 10.1. Đánh giá Định tính qua các kịch bản kiểm thử (Case Studies)
...
### 10.2. Đánh giá Hiệu năng Hệ thống
